...
from matsar.algorithms.utils.util import check, init
from matsar.algorithms.utils.transformer_act import discrete_autoregreesive_act
from matsar.algorithms.utils.transformer_act import discrete_parallel_act
from matsar.algorithms.utils.transformer_act import continuous_autoregreesive_act
from matsar.algorithms.utils.transformer_act import continuous_parallel_act
from matsar.algorithms.utils.transformer_act import multidiscrete_autoregreesive_act
from matsar.algorithms.utils.transformer_act import multidiscrete_parallel_act
from matsar.algorithms.utils.channel_vit import ChannelVisionTransformer
...
class MultiAgentTransformer(nn.Module):
    ...
    def forward(self, state, obs, action, available_actions=None, active_agents=None):
        # state: (batch, n_agent, state_dim)
        # obs: (batch, n_agent, obs_dim)
        ...
        if self.action_type == 'Discrete':
            ...
            action_log, entropy = discrete_parallel_act(self.decoder, obs_rep, obs, action, batch_size,
                                                        self.n_agent, self.action_dim, self.tpdv, available_actions, active_agents)
        elif self.action_type == "MultiDiscrete":
            action = action.long()
            action_log, entropy = multidiscrete_parallel_act(self.decoder, obs_rep, obs, action, batch_size,
                                                        self.n_agent, self.action_dim, self.tpdv, available_actions, active_agents) #action_log and entropy are lists
        else:
    ...
    def get_actions(self, state, obs, available_actions=None, deterministic=False, active_agents=None):
        # state unused
        ...
        if self.action_type == "Discrete":
            output_action, output_action_log = discrete_autoregreesive_act(self.decoder, obs_rep, obs, batch_size,
                                                                           self.n_agent, self.action_dim, self.tpdv,
                                                                           available_actions, deterministic, active_agents)
        elif self.action_type == "MultiDiscrete":
            output_action, output_action_log = multidiscrete_autoregreesive_act(self.decoder, obs_rep, obs, batch_size,
                                                                           self.n_agent, self.action_dim, self.tpdv,
                                                                           available_actions, deterministic, active_agents)
        else:
