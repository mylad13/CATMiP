import torch
import numpy as np
from matsar.utils.util import update_linear_schedule
from matsar.utils.util import get_shape_from_obs_space, get_shape_from_act_space
from matsar.algorithms.utils.util import check
from matsar.algorithms.mat.algorithm.ma_transformer import MultiAgentTransformer
...
class TransformerPolicy:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, num_agents, n_agent_types, device=torch.device("cpu")):
        self.device = device
        ...
        if self.algorithm_name in ["mat", "mat_dec"]:
            ...
            from matsar.algorithms.mat.algorithm.mat_decoder import MultiAgentDecoder as MAT
        elif self.algorithm_name == "mat_encoder":
            from matsar.algorithms.mat.algorithm.mat_encoder import MultiAgentEncoder as MAT
        else:
            raise NotImplementedError

        self.transformer = MAT(self.share_obs_dim, self.obs_dim, self.act_dim, num_agents, n_agent_types,
                               n_block=args.n_block, n_embd=args.n_embd, n_embd_vit=args.n_embd_vit, n_head=args.n_head,
    ...
    def evaluate_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, actions, masks,
        ...
        action_log_probs, values, entropy = self.transformer(cent_obs, obs, actions, available_actions, active_agents)
