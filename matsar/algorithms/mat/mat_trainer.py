...
from matsar.utils.util import get_gard_norm, huber_loss, mse_loss
from matsar.utils.valuenorm import ValueNorm
from matsar.algorithms.utils.util import check
from matsar.algorithms.utils.util import get_connected_agents
...
class MATTrainer:
    ...
    def ppo_update(self, sample): #TODO: make sure active_masks are used correctly to mask out info after envs are done
        ...
        if self.use_partial_comm:
            ...
            pre_action_log_probs = pre_action_log_probs.view(-1, self.num_agents, 2)
            values = np.zeros((batch_size, self.num_agents, 1))
            action_log_probs = np.zeros((batch_size, self.num_agents, 2))
            ...
            action_log_probs = action_log_probs.view(-1, 2)
