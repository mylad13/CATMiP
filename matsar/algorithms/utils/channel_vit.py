...
from matsar.algorithms.utils.vit import Block
from matsar.algorithms.utils.util import trunc_normal_
...
class AttentionPooling(nn.Module):
    def __init__(self, embed_dim):
        super().__init__()
        self.cls_vec = nn.Parameter(torch.randn(embed_dim))
        self.fc = nn.Linear(embed_dim, embed_dim)
        self.softmax = nn.Softmax(-1)

    def forward(self, x):
        # print("shape of x before matmul is", x.shape)
        weights = torch.matmul(x, self.cls_vec)
        # print("shape of weights is ", weights.shape)
        weights = self.softmax(weights.unsqueeze(-1))
        # print("shape of weights after softmax is ", weights.shape)
        x = torch.einsum("b n i, b n d -> b d", weights,x)
...
class ChannelVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Output layer
        self.head = (
            nn.Linear(embed_dim, output_embed_dim) if embed_dim != output_embed_dim else nn.Identity()
        )
