...
class PatchEmbedPerChannel(nn.Module):
    ...
    def forward(self, x, extra_tokens={}):
        ...
        # print("shape of channel_embed", self.channel_embed.shape)
...
class ChannelVisionTransformer(nn.Module):
    ...
    def prepare_tokens(self, x, extra_tokens):
        ...
        # commented out because we are not using the cls token
    ...
    def forward(self, x, extra_tokens={}):
        ...
        for blk in self.blocks:
            x = blk(x)
        x = self.norm(x)
        return x[:, 0] # This should be changed if we're not classifying
