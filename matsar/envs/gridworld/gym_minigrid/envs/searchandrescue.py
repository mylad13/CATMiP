...
import time
from matsar.envs.gridworld.gym_minigrid.minigrid import *
...
from matsar.envs.gridworld.frontier.apf import APF
from matsar.envs.gridworld.frontier.utility import utility_goal
from matsar.envs.gridworld.frontier.rrt import rrt_goal
from matsar.envs.gridworld.frontier.nearest import nearest_goal
from matsar.envs.gridworld.frontier.voronoi import voronoi_goal

from matsar.envs.gridworld.gym_minigrid.register import register

from matsar.utils import astar
from matsar.utils import dstarlite, plotting, env
...
class SearchAndRescueEnv(MultiRoomEnv):
    ...
    def overall_gen_grid(self, width, height):
                    ...
                    if self._rand_float(0, 1) < 0.1: # and i != 0 and i != w-2:
                        ...
                        for s in range(self.door_size):
                            ...
                            if self.block_doors and self._rand_float(0, 1) < self.block_chance: #randomly block some doorways
                                ...
                                self.place_obj(Rubble(), top = (pos[0]-1, pos[1]-1), size = (2,2), reject_fn=accept_near_doors)
                            pos = (pos[0], pos[1] + 1)
        ...
        if self._goal_default_pos is not None:
            ...
            initial_trace = self.place_obj(Trace(), reject_fn = reject_far_from_target)
        
        if initial_trace[0] != -1:
            # Generate a path for the target to somewhere close
            path_planner = astar.AStar(obs_list, tuple(initial_trace), tuple(self.target_pos), "manhattan")
            path, _ = path_planner.searching()
    ...
    def set_target_each_map(self, agent_id):
        ...
        if self.target_found[agent_id]:
            # adjacent_cells = self.adjacent_cells(*self.target_pos)
            self.target_each_map[agent_id][self.target_pos[1]+self.agent_view_size, self.target_pos[0]+self.agent_view_size] = 1 #1/2
            # for cell in adjacent_cells:
            #     self.target_each_map[agent_id][cell[1]+self.agent_view_size, cell[0]+self.agent_view_size] = 1 #1/8
    ...
    def set_target_all_map(self):
        ...
        if self.is_target_found:
            # adjacent_cells = self.adjacent_cells(*self.target_pos)
            self.target_all_map[self.target_pos[1]+self.agent_view_size, self.target_pos[0]+self.agent_view_size] = 1 #1/2
            # for cell in adjacent_cells:
            #     self.target_all_map[cell[1]+self.agent_view_size, cell[0]+self.agent_view_size] = 1 #1/8
        

    def set_rubble_each_map(self, agent_id): # unused in current implementation
        ...
        for rubble_pos in self.agent_rubble_sets[agent_id]:
            adjacent_cells = self.adjacent_cells(*rubble_pos)
            self.rubble_each_map[agent_id][rubble_pos[1]+self.agent_view_size, rubble_pos[0]+self.agent_view_size] = 1 #1/2
            for cell in adjacent_cells:
                self.rubble_each_map[agent_id][cell[1]+self.agent_view_size, cell[0]+self.agent_view_size] = 1 #1/8
    
    def set_rubble_all_map(self): # unused in current implementation
        ...
        for rubble_pos in self.all_rubble_set:
            ...
            for cell in adjacent_cells:
                self.rubble_all_map[cell[1]+self.agent_view_size, cell[0]+self.agent_view_size] = 1 #1/8
    ...
    def reset(self):
        ...
        for index in range(self.num_agents):
            if self.original_agent_types_list[index] == 10: # replace the code 10 agent randomly with 0 or 1
    ...
    def step(self, action):
        ...
        step_reward_each_map = np.zeros(
            (self.num_agents, self.width + 2*self.agent_view_size, self.height + 2*self.agent_view_size))
        overlap_map = np.zeros(
            (self.width + 2*self.agent_view_size, self.height + 2*self.agent_view_size))
        weighted_step_reward_each_map = np.zeros(
            (self.num_agents, self.width + 2*self.agent_view_size, self.height + 2*self.agent_view_size))
        explored_all_map = np.zeros((self.width + 2*self.agent_view_size,
                                    self.height + 2*self.agent_view_size))
        obstacle_all_map = np.zeros((self.width + 2*self.agent_view_size,
        ...
        for i in range(self.num_agents):
            ...
            # Agent i has found the target
            if self.is_target_found == 0 and target_pos != None:
                self.target_found[i] = 1
                self.is_target_found = 1
