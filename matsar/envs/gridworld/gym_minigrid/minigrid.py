...
class MiniGridEnv(gym.Env):
    ...
    def step(self, action):
        ...
        for agent_id in range(self.num_agents):
            # Check if the agent can take an action at this time step
            ...
            # Move forward
                ...
                if fwd_cell == None or fwd_cell.can_overlap():
                    can_pass = True
                    # for j in range(self.num_agents): # use when agents can't pass through each other (needs a fix: when twp agents face each other, they might get stuck)
                    #     if j != agent_id:
                    ...
                    if can_pass:
                        ...
                        if self.use_energy_penalty:
                            reward[agent_id] += 0.1*self._penalty() #energy cost of moving
                        if self.agent_types_list[agent_id] == 0:
                            self.agents_time_until_action[agent_id] = 1
                        # elif self.agent_types_list[agent_id] == 2:
                        #     self.agents_time_until_action[agent_id] = 1
            ...
            # Stop
                ...
                if self.agent_types_list[agent_id] == 0 and self.target_found[agent_id]: #for actuator agents
                        ...
                        # _, dist_to_target = self.astar_path(self.agent_pos[agent_id], self.target_pos)
                        alpha = 1/self.num_agents
                        if l1dist_to_target <=1 and self.target_rescued == 0:
                            # reward += alpha*400*self._reward() # Team's success reward
                            # reward[agent_id] += (1-alpha)*400*self._reward() # agent's success reward
                            reward += 300*self._reward() # Team's success reward
                            # reward[agent_id] += 150*self._reward() # agent's success reward
                            self.target_rescued = 1
                            # print("agent {} has stayed at the target position".format(agent_id))
                        else:
                            pass
            ...
            if self.agent_types_list[agent_id] == 0 and self.target_found[agent_id] and self.target_rescued == 0:
                # dist[agent_id] = euclideandistance(self.agent_pos[agent_id], self.target_pos) #TODO: add support for multiple targets
                l1dist_to_target = l1distance(self.agent_pos[agent_id], self.target_pos) 
                # _, dist_to_target = self.astar_path(self.agent_pos[agent_id], self.target_pos)
                # print("distance to target is {}".format(dist_to_target))
                if l1dist_to_target>=self.prev_dist[agent_id]:
                    reward[agent_id] += self._penalty()
                else:
                    pass
                self.prev_dist[agent_id] = l1dist_to_target
    ...
    def render(self, mode='human', short_goal_pos=None, close=False, highlight=True, tile_size=TILE_PIXELS, first=False):
        ...
        if not self.window:
            from matsar.envs.gridworld.gym_minigrid.window import Window
