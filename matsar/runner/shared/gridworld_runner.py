...
from matsar.utils.util import update_linear_schedule, get_shape_from_act_space, get_shape_from_obs_space, AsynchControl
from matsar.runner.shared.base_runner import Runner
...
class GridWorldRunner(Runner):
    ...
    def run(self):
        ...
        if self.asynch:
            ...
            self.asynch_control = AsynchControl(num_envs=self.n_rollout_threads, num_agents=self.num_agents,
                                                limit=self.episode_length, random_fn=generate_random_period, min_wait=3, max_wait=5, rest_time = 30)
        ...
        if self.use_full_comm:
            for e in range(len(dict_obs)):
                for agent_id in range(self.num_agents):
                    ...
                    # agent_pos_map[e, agent_id, current_agent_pos[e,agent_id][0]-1:current_agent_pos[e,agent_id][0]+2,
                ...
                for agent_id in range(self.num_agents):
                    for j in range(self.num_agents):
                        if j != agent_id:
                            ...
                            # elif self.agent_types_list[j] == 2:
            ...
            for e in range(len(dict_obs)):
                for agent_id in range(self.num_agents):
                    ...
                    obs['global_agent_map'][e, agent_id, 1] = infos[e]['occupied_all_map'][self.agent_view_size:self.full_w -
                                                                    self.agent_view_size, self.agent_view_size:self.full_w-self.agent_view_size]
                    obs['global_agent_map'][e, agent_id, 2] = infos[e]['target_all_map'][self.agent_view_size:self.full_w -
                                                                    self.agent_view_size, self.agent_view_size:self.full_w-self.agent_view_size]
                    # obs['global_agent_map'][e, agent_id, 3] = infos[e]['rubble_all_map'][self.agent_view_size:self.full_w -
                    #                                                     self.agent_view_size, self.agent_view_size:self.full_w-self.agent_view_size]
                    ...
                    obs['global_agent_map'][e, agent_id, 4] = agent_pos_map[e, agent_id][self.agent_view_size:self.full_w -
                                                                    self.agent_view_size, self.agent_view_size:self.full_w-self.agent_view_size]
                    ...
                    obs['global_agent_map'][e, agent_id, 5] = type0_agent_pos_map[e,agent_id][self.agent_view_size:self.full_w -
                                                                    self.agent_view_size, self.agent_view_size:self.full_w-self.agent_view_size]
                    obs['global_agent_map'][e, agent_id, 6] = type1_agent_pos_map[e,agent_id][self.agent_view_size:self.full_w -
                                                                    self.agent_view_size, self.agent_view_size:self.full_w-self.agent_view_size]
                    # obs['global_agent_map'][e, agent_id, 8] = type2_agent_pos_map[e,agent_id][self.agent_view_size:self.full_w -
                                                                                #    self.agent_view_size, self.agent_view_size:self.full_w-self.agent_view_size]
                    obs['global_agent_map'][e, agent_id, 7] = self.agent_goal_history[e,agent_id][self.agent_view_size:self.full_w -
                                                                    self.agent_view_size, self.agent_view_size:self.full_w-self.agent_view_size]
                    ...
                    obs['local_agent_map'][e,agent_id,2] = infos[e]['target_all_map'][current_agent_pos[e, agent_id][0]-self.action_size:current_agent_pos[e, agent_id][0]
                                                                +self.action_size+1, current_agent_pos[e, agent_id][1]-self.action_size:current_agent_pos[e, agent_id][1]+self.action_size+1]
                    obs['local_agent_map'][e,agent_id,3] = self.agent_goal_history[e,agent_id][current_agent_pos[e, agent_id][0]-self.action_size:current_agent_pos[e, agent_id][0]
                                                                +self.action_size+1, current_agent_pos[e, agent_id][1]-self.action_size:current_agent_pos[e, agent_id][1]+self.action_size+1]
            ...
            obs['local_agent_map'] = np.zeros((len(dict_obs), self.num_agents, 4, 2*self.action_size + 1, 2*self.action_size + 1), dtype=np.float32)
            agent_pos_map = np.zeros((len(dict_obs), self.num_agents, self.full_w, self.full_h), dtype=np.float32)
            # agent_short_goal_map = np.zeros((len(dict_obs), self.num_agents, self.full_w-2*self.agent_view_size, self.full_h-2*self.agent_view_size), dtype=np.float32)
            # agent_trajectory_map = np.zeros((len(dict_obs), self.num_agents, self.full_w-2*self.agent_view_size, self.full_h-2*self.agent_view_size), dtype=np.float32)
            other_agents_pos_map = np.zeros((len(dict_obs), self.num_agents, self.full_w, self.full_h), dtype=np.float32)
            ...
            for e in range(len(dict_obs)):
                for agent_id in range(self.num_agents):                   
                    ...
                    #         coord = np.array([x, y]) + self.agent_pos[e, agent_id]
                    #         if coord[0] < 0 or coord[0] >= self.map_size or coord[1] < 0 or coord[1] >= self.map_size: #map boundaries
                    #             pass
                    #         else:
                    #             highlighted_action_area[e, agent_id, int(coord[1]), int(coord[0])] = 1
                    # highlighted_action_area[e,agent_id, self.agent_pos[e, agent_id][1]:self.agent_pos[e, agent_id][1] + 2*self.action_size + 1,
                    #                         self.agent_pos[e, agent_id][0]:self.agent_pos[e, agent_id][0] + 2*self.action_size + 1] = 1 
                    # agent_short_goal_map[e, agent_id, self.macro_action[e, agent_id][0],
                    #                 self.macro_action[e, agent_id][1]] = 1
            ...
            for e in range(len(dict_obs)):
                for agent_id in range(self.num_agents):
                    ...
                    obs['global_agent_map'][e, agent_id, 1] = infos[e]['occupied_each_map'][agent_id][self.agent_view_size:self.full_w -
                                                                        self.agent_view_size, self.agent_view_size:self.full_w-self.agent_view_size]
                    obs['global_agent_map'][e, agent_id, 2] = infos[e]['target_each_map'][agent_id][self.agent_view_size:self.full_w -
                                                                    self.agent_view_size, self.agent_view_size:self.full_w-self.agent_view_size]
                    # obs['global_agent_map'][e, agent_id, 3] = infos[e]['rubble_each_map'][agent_id][self.agent_view_size:self.full_w -
                    #                                                     self.agent_view_size, self.agent_view_size:self.full_w-self.agent_view_size]
                    obs['global_agent_map'][e, agent_id, 3] = infos[e]["each_agent_trajectory_map"][agent_id][self.agent_view_size:self.full_w - self.agent_view_size,
                                                                                            self.agent_view_size:self.full_h - self.agent_view_size]                    
                    obs['global_agent_map'][e, agent_id, 4] = agent_pos_map[e, agent_id][self.agent_view_size:self.full_w -
                    ...
                    obs['global_agent_map'][e, agent_id, 6] = type1_agent_pos_map[e,agent_id][self.agent_view_size:self.full_w -
                                                                                   self.agent_view_size, self.agent_view_size:self.full_w-self.agent_view_size]
                    # obs['global_agent_map'][e, agent_id, 7] = type2_agent_pos_map[e,agent_id][self.agent_view_size:self.full_w -
                    #                                                                self.agent_view_size, self.agent_view_size:self.full_w-self.agent_view_size]
                    obs['global_agent_map'][e, agent_id, 7] = self.agent_goal_history[e,agent_id][self.agent_view_size:self.full_w -
                                                                    self.agent_view_size, self.agent_view_size:self.full_w-self.agent_view_size]
                    # obs['global_agent_map'][e, agent_id, 6] = other_agents_pos_map[e,agent_id][self.agent_view_size:self.full_w -
    ...
    def compute_global_goal(self, step):
        ...
        def get_short_term_goal(self, concat_obs, concat_share_obs, n_threads, available_actions, active_agents = None):
            value, action, action_log_prob, _ , _  = self.trainer.policy.get_actions(concat_share_obs,
                                                                                    concat_obs,
            ...
            return values, actions, action_log_probs, short_term_goal
        ...
        if self.asynch:
            if self.use_full_comm: # used for centralized and asynchronous training with full communication
                ...
                values, actions, action_log_probs, short_term_goal \
                        = get_short_term_goal(self, concat_obs, concat_share_obs, n_stacked_threads, all_available_actions)
                ...
                for e in range(n_threads):
                    ...
                    for agent_id in active_threads[e][1:]:
                        ...
                        agent_num += 1
                    counter += 1
            elif self.use_partial_comm: # used for centralized and asynchronous training with partial communication (not used anymore)
                ...
                for key in obs_shape:
                    ...
                    concat_obs[key] = np.concatenate(concat_obs[key])
                values, actions, action_log_probs, short_term_goal \
                        = get_short_term_goal(self, concat_obs, concat_share_obs, n_stacked_threads)
                counter = 0 # used to move through the concatenated actions resulting from concatenated obs
                for e in range(n_threads):
                    if len(active_threads[e]) <= 1:
                        continue
            ...
            if self.use_full_comm: # used for centralized and synchronous training with full communication
                ...
                returned_values, returned_actions, returned_action_log_probs, short_term_goal \
                        = get_short_term_goal(self, concat_obs, concat_share_obs, n_threads, self.available_actions)
        ...
        returned_rnn_states = np.zeros((self.n_rollout_threads, self.num_agents, self.recurrent_N, self.hidden_size)) # this is not used in MAT
        returned_rnn_states_critic = np.zeros_like(returned_rnn_states) # this is not used in MAT
        return returned_values, returned_actions, returned_action_log_probs, returned_rnn_states, returned_rnn_states_critic
    
    def eval_compute_global_goal(self, infos):
        if self.use_render:
            n_threads = self.n_rollout_threads
        elif self.use_eval:
        ...
        # Get short-term goals.
        def get_short_term_goal(self, concat_obs, concat_share_obs, n_threads, available_actions = None):
            # available_actions=None                
            if self.use_render:
        ...
        if self.use_partial_comm: 
            ...
            if self.asynch: # Asynchronous and partial communication
                ...
                for key in obs.keys():
                    ...
                    concat_obs[key] = np.concatenate(concat_obs[key])
                all_available_actions = np.array(all_available_actions)
                short_term_goal = get_short_term_goal(self, concat_obs, concat_share_obs, n_stacked_threads, all_available_actions)
            ...
            if self.asynch: #asynchronous and full communication
                ...
                short_term_goal = get_short_term_goal(self, concat_obs, concat_share_obs, n_threads, self.available_actions)
                short_term_goal = short_term_goal.squeeze() + self.agent_pos
                self.macro_action = self.correct_ma_bounds(short_term_goal)
    ...
    def eval(self):
        ...
        for episode in range(self.all_args.eval_episodes):
            ...
            self.eval_rnn_states = np.zeros((self.n_eval_rollout_threads, *self.buffer.rnn_states.shape[2:]), dtype=np.float32)
            ...
            if self.use_action_masking:
                self.available_actions = self.get_available_actions(self.eval_envs)
            else:
                self.available_actions = None
            ...
            self.eval_compute_global_goal(eval_infos)
        ...
        # Save data to file
        np.save('mission_completion_data.npy', mission_completion_data)
    ...
    def render(self):
        ...
        for episode in range(self.all_args.render_episodes):
            ...
            obs = self._convert(dict_obs, infos)
            self.obs = obs
            self.eval_compute_global_goal(infos)
            period_rewards = np.zeros((self.n_rollout_threads, self.num_agents, 1))
