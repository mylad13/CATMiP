...
from matsar.utils.util import check,get_shape_from_obs_space, get_shape_from_act_space, get_connected_agents
...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, share_obs_space, act_space, env_name):
        ...
        self.was_agent_active = np.full((self.episode_length + 1, self.n_rollout_threads, num_agents), False, dtype=np.bool)
    ...
    def async_insert(self, share_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
        ...
        for e, a, agent_step in active_agents:
            ...
            self.was_agent_active[self.step + 1 , e, a] = True * self.masks[agent_step + 1, e, a].astype(np.bool)
    ...
    def update_mask(self, steps): #only called at the end of episode, before compute
        ...
        self.active_masks = self.active_masks * self.masks #same as mask for now (might change when implementing agent loss during the mission)
    ...
    def after_update(self):
        ...
        if self.asynch:
            ...
            self.was_agent_active = np.full((self.episode_length + 1, self.n_rollout_threads, self.num_agents), False, dtype=np.bool)
            self.was_agent_active[0] = True
